...
myBalanceUSDT = 1000000.0
myFirstBalanceUSDT = myBalanceUSDT
...
pairs = [
   {
        ...
        'spend_sum': 2000.0,  # Сколько тратить base каждый раз при покупке quote
        'profit_markup': 0.0001, # Какой навар нужен с каждой сделки? (0.001 = 0.1%)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("{path}/{fname}.log".format(path=os.path.dirname(os.path.abspath(__file__)), fname="binance")),
        logging.StreamHandler(sys.stdout)
    ])
unicode_string = unicode("\xc3\xb4", "utf-8")

FIELD_STYLES_DEBUG = dict(
...
coloredlogs.install(level='DEBUG' ,fmt="%(asctime)s [%(levelname)-5.5s] %(message)s",use_chroot=False,
    field_styles=FIELD_STYLES_DEBUG,
...
while True:
    try:
        ...
        for row in cursor.execute(orders_q):
            orders_info[str(row[0])] = {'order_type': row[1], 'order_pair': row[2],'order_status': row[3] ,'sell_amount': row[4], 'sell_price': row[5],
                                         'buy_created': row[6], 'buy_amount': row[7], 'buy_price': row[8] }
        ...
        if orders_info:
            ...
            for order in orders_info:
                ...
                order_status = orders_info[order]['order_status']
                log.debug("Состояние ордера {order} - {status}".format(order=order, status=order_status))
                if order_status == 'NEW':
                   log.debug('Ордер {order} всё еще не выполнен'.format(order=order))
                ...
                if orders_info[order]['order_type'] == 'buy':
                    ...
                    if order_status == 'FILLED':
                        ...
                        """.format(
                            ...
                            order=order, exec_qty=float(my_amount)
                        ))
                        ...
                        need_to_earn = orders_info[order]['buy_amount']*orders_info[order]['buy_price']*(1+all_pairs[orders_info[order]['order_pair']]['profit_markup'])
                    ...
                    # Ордер еще не исполнен, частичного исполнения нет, проверяем возможность отмены
                        ...
                        if time_passed > BUY_LIFE_TIME_SEC:
                            ...
                            cancel = {'orderId' : new_order[orderId] }
                ...
                if curr_rate >= need_price:
                    ...
                    if 'orderId' in new_order:
                        ...
                        conn.commit()
                        num_deal = num_deal+1
                        myBalanceUSDT = myBalanceUSDT + need_to_earn
                        #log.info(("Баланс : $" +format_decimal(myBalanceUSDT, locale='en_US')).replace('$-','-$'))
                        #log.info(("Прибыль с момента запуска : " + format_decimal(myBalanceUSDT-myFirstBalanceUSDT, locale='en_US')).replace('$-','-$'))
                        #log.info(("Количество сделок: "+str(num_deal)))
        ...
        if all_pairs:
            ...
            for pair_name, pair_obj in all_pairs.items():
                ...
                if myBalanceUSDT >= (pair_obj['spend_sum']):
                    ...
                    try:
                        ...
                        if 'orderId' in new_order:
                            ...
                            if 'orderId' in new_order:
                                log.info("Ордер на покупку выполнен : {new_order}".format(new_order=new_order))
                                cursor.execute(
                                        """
                                          UPDATE orders
                                          SET
                            ...
                            myBalanceUSDT = myBalanceUSDT - trade_am
                            log.info(("Баланс : $" +format_decimal(myBalanceUSDT, locale='en_US')).replace('$-','-$'))
